...
export class SurveyService extends DefaultService{
  ...
  getSurveysBySessionId(sessionId: number){
    return firstValueFrom(this.http.get<Survey[]>(this.getAPIUrl()+`survey-service/presenter/v1/session/${sessionId}/surveys`)
      .pipe(
        map((values: Survey[]) =>
  ...
  getTemplatesBySessionId(sessionId: number){
    return firstValueFrom(this.http.get<SurveyTemplate[]>(this.getAPIUrl()+`survey-service/presenter/v1/session/${sessionId}/survey/templates`)
      .pipe(
  ...
  createByTemplate(sessionId: number, template: ISurveyTemplate){
    return firstValueFrom(this.http.post<SurveyTemplate>(this.getAPIUrl()+`survey-service/presenter/v1/session/${sessionId}/survey/create`, template)
      .pipe(
  ...
  createByTemplateId(sessionId: number, id: number){
    return firstValueFrom(this.http.get(this.getAPIUrl()+`survey-service/presenter/v1/session/${sessionId}/survey/create/${id}`));
  }

  sendAnswer(sessionId: number, surveyId: number, value: string){
    return firstValueFrom(this.http.post(this.getAPIUrl()+`survey-service/participant/v1/session/${sessionId}/survey/${surveyId}/answer`, {text: value}));
  }
